...
if ($contactSecret -and $careerSecret) {
    Write-Host "OK: Found Turnstile secrets in environment" -ForegroundColor Green
    ...
    Write-Host "WARNING: Turnstile secrets not found in environment" -ForegroundColor Yellow
...
Write-Host "NOTE: Extensions API errors are non-critical and can be ignored" -ForegroundColor Yellow
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "WARNING: Deployment exited with errors, but checking if functions deployed anyway..." -ForegroundColor Yellow
    firebase functions:list
    Write-Host ""
    Write-Host "TIP: If functions are listed above, they deployed successfully despite the error." -ForegroundColor Cyan
}
